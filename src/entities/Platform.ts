import { PALETTE, TILE_SIZE, randomRange } from '../core/constants';
import type { Rect } from '../core/constants';
import type { ParticleSystem } from '../systems/ParticleSystem';
import type { AudioManager } from '../audio/AudioManager';

export type PlatformType = 'solid' | 'crumbling' | 'moving' | 'rewind_only' | 'spike' | 'bouncy';
...
export class Platform {
  ...
  static readonly COLLAPSE_TIME = 1.5;
  static readonly RESPAWN_TIME = 6;

  constructor(
    x: number, y: number, w: number, h: number,
    type: PlatformType = 'solid',
    opts: { moveSpeed?: number; moveRange?: number; moveAxis?: 'x' | 'y'; spikeCount?: number } = {}
  ) {
    this.x = x;
    this.y = y;
    this.w = w;
  ...
  restoreState(s: PlatformState): void {
    this.x = s.x; this.y = s.y; this.crackLevel = s.crackLevel;
    this.collapseTimer = s.collapseTimer; this.collapsed = s.collapsed;
    this.respawnTimer = s.respawnTimer; this.moveT = s.moveT; this.active = s.active;
  }

  update(dt: number, particles: ParticleSystem, audio: AudioManager): void {
    if (this.type === 'moving') {
      this.moveT += dt * this.moveSpeed / this.moveRange;
      const offset = Math.sin(this.moveT) * this.moveRange;
      if (this.moveAxis === 'x') this.x = this.originX + offset;
  ...
  crack(particles: ParticleSystem, audio: AudioManager): void {
    if (this.type !== 'crumbling' || this.collapsed) return;
    ...
    if (this.crackLevel !== prev) {
      ...
      if (this.crackLevel === 1) this.collapseTimer = Platform.COLLAPSE_TIME;
    }
  }

  collapse(particles: ParticleSystem, audio: AudioManager): void {
    this.collapsed = true;
    this.active = false;
  ...
  draw(ctx: CanvasRenderingContext2D, time: number, isRewinding: boolean): void {
    if (this.collapsed) return;
    if (!this.active && this.type !== 'crumbling') return;
  ...
  private drawRewindOnly(ctx: CanvasRenderingContext2D, time: number, isRewinding: boolean): void {

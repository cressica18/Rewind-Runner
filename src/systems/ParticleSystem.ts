import { Vec2, randomRange, lerp } from '../core/constants';
...
export class ParticleSystem {
  ...
  burst(x: number, y: number, count: number, color: string, speed = 200, life = 0.5, size = 4, glow = false): void {
  ...
  burstDir(x: number, y: number, count: number, color: string, dirX: number, dirY: number, speed = 150, spread = 1, life = 0.4, size = 3): void {
    for (let i = 0; i < count; i++) {
  ...
  rewindTrail(x: number, y: number): void {
    this.emit({
      x: x + randomRange(-8, 8),
      y: y + randomRange(-8, 8),
  ...
  getSnapshot(): Particle[] {
    return this.particles.map(p => ({ ...p }));
  }

  restoreSnapshot(snap: Particle[]): void {

...
export type MenuResult = {
  action: 'play' | 'controls' | 'levelselect' | 'resume' | 'mainmenu' | 'restart' | 'nextlevel' | 'selectlevel';
  level?: number;
};
...
export class MenuSystem {
  ...
  drawMainMenu(ctx: CanvasRenderingContext2D, hoveredBtn: number): MenuResult | null {
    ...
    const buttons = [
      ...
      { label: 'LEVEL SELECT', action: 'levelselect' as const },
      { label: 'CONTROLS', action: 'controls' as const },
    ];
  ...
  drawButton(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, label: string, hovered: boolean): void {
    ctx.save();
  ...
  drawPauseMenu(ctx: CanvasRenderingContext2D, hoveredBtn: number): void {
  ...
  drawLevelComplete(ctx: CanvasRenderingContext2D, data: {
    levelName: string; time: number; collectibles: number;
    totalCollectibles: number; rewindsUsed: number; stars: number;
    score: number; isLastLevel: boolean; hoveredBtn: number;
  }): void {
  ...
  drawGameOver(ctx: CanvasRenderingContext2D, hoveredBtn: number): void {
  ...
  drawControls(ctx: CanvasRenderingContext2D, hoveredBtn: number): void {
    this.drawBackground(ctx);
  ...
  drawLevelSelect(ctx: CanvasRenderingContext2D, hoveredBtn: number, unlockedLevels: number, levelData: { name: string; subtitle: string; stars: number }[]): void {
    this.drawBackground(ctx);
    ctx.textAlign = 'center';
    ctx.font = 'bold 32px "Courier New", monospace';
    ctx.fillStyle = PALETTE.text;
  ...
  getButtonIndex(mx: number, my: number, buttons: { x: number; y: number; w: number; h: number }[]): number {
    for (let i = 0; i < buttons.length; i++) {
      const b = buttons[i];

...
import type { WorldSnapshot } from './systems/RewindSystem';
...
export class World {
  ...
  playerDead = false;
  deathTimer = 0;
  readonly DEATH_RESPAWN_TIME = 2.0;

  // Level info
  levelWidth: number;
  levelHeight: number;

  constructor(config: LevelConfig, audio: AudioManager) {
  ...
  private resolveCollisions(dt: number, audio: AudioManager): void {
    const p = this.player;

    for (const platform of this.platforms) {
      if (!platform.active || platform.collapsed) continue;

      if (platform.type === 'rewind_only' && !this.rewind.isRewinding) continue;
      ...
      if (!rectOverlap(pr, plr)) continue;
    ...
    for (const door of this.doors) {
      const dr = door.rect;
      if (dr.h <= 0) continue;
      if (!rectOverlap(p.rect, dr)) continue;
      ...
      if (minY < minX) {
        ...
        if (overlapLeft < overlapRight) p.x = dr.x - p.w;
        else p.x = dr.x + dr.w;
        p.vx = 0;
      }
    }
  ...
  private handleInteractions(wantsInteract: boolean, audio: AudioManager): void {
    const p = this.player;
  ...
  private applySnapshot(snap: WorldSnapshot): void {
    ...
    for (let i = 0; i < this.platforms.length; i++) {
      if (snap.platforms[i]) this.platforms[i].restoreState(snap.platforms[i]);
    }
    for (let i = 0; i < this.hazards.length; i++) {
      if (snap.hazards[i]) this.hazards[i].restoreState(snap.hazards[i]);
    }
    for (let i = 0; i < this.collectibles.length; i++) {
      if (snap.collectibles[i]) this.collectibles[i].restoreState(snap.collectibles[i]);
    }
    for (let i = 0; i < this.switches.length; i++) {
      if (snap.switches[i]) this.switches[i].restoreState(snap.switches[i]);
    }
    for (let i = 0; i < this.doors.length; i++) {
      if (snap.doors[i]) this.doors[i].restoreState(snap.doors[i]);
    }
    this.portal.restoreState(snap.portal);
    for (let i = 0; i < this.checkpoints.length; i++) {
      if (snap.checkpoints[i]) this.checkpoints[i].restoreState(snap.checkpoints[i]);
    }
